...
{
    internal class SemErrorOnlyOneHexProperty : NodeVisitor<IEnumerable<DiagnosticResult>>
    {
        ...
        public override IEnumerable<DiagnosticResult> VisitProperty(PropertyNode propertyNode)
        {
            ...
            int count = complexNode.properties.Where(n => n.identifier.GetType() == typeof(HexKeywordNode)).Count();

            if (count > 1)
            {
                yield return new DiagnosticResult(Severity.Error, $"Shape may only contan one hex property!");
            }
        }
    }
}
