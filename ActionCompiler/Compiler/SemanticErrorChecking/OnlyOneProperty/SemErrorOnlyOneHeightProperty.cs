...
{
    internal class SemErrorOnlyOneHeightProperty : NodeVisitor<IEnumerable<DiagnosticResult>>
    {
        ...
        public override IEnumerable<DiagnosticResult> VisitProperty(PropertyNode propertyNode)
        {
            ...
            int count = complexNode.properties.Where(n => n.identifier.GetType() == typeof(HeightKeywordNode)).Count();

            if (count > 1)
            {
                yield return new DiagnosticResult(Severity.Error, $"Shape may only contan one height property!");
            }
        }
    }
}
